...
const ServiceTypesPage: React.FC = () => {
  ...
  const openModal = (service: ServiceType | null = null) => {
    if (service) {
      ...
      setFormState({ ...service, requirements: [...service.requirements], allowedPackageSizes: service.allowedPackageSizes ? [...service.allowedPackageSizes] : [] });
    } else {
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {pageItems.map(service => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center rounded-full border px-3 py-0.5 text-xs font-semibold ${statusBadge[service.status]}`}
                      >
                        {service.status}
                      </span>
                    </td>
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/80 shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              {selectedService && (
                <span
                  className={`inline-flex items-center rounded-full border px-3 py-0.5 text-[11px] font-semibold ${statusBadge[selectedService.status]}`}
                >
