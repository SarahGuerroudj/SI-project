...
const DestinationsPage: React.FC = () => {
  ...
  const [formState, setFormState] = useState<Omit<DestinationRecord, 'id'>>({
    name: '',
    country: '',
    city: '',
    deliveryZone: '',
      ...
      <div className="relative z-10 p-6 space-y-6 text-slate-900 dark:text-slate-100 transition-colors duration-500">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className={`flex items-center gap-2 rounded-full border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-900/70 backdrop-blur px-4 py-2 text-sm font-medium transition ${isThemeTransitioning ? 'ring-2 ring-lime-400/40 dark:ring-lime-500/40' : ''}`}
            >
              {theme === 'dark' ? <Sun size={16} /> : <Moon size={16} />}
              <span>{theme === 'dark' ? 'Light mode' : 'Dark mode'}</span>
              <span className="relative h-2 w-8 rounded-full bg-slate-200 dark:bg-slate-700">
                <span
                  className={`absolute top-[-2px] h-4 w-4 rounded-full bg-lime-500 transition-all ${theme === 'dark' ? 'left-4' : 'left-0.5'}`}
                ></span>
              </span>
            </button>
            <button
        ...
        <div className="grid gap-5 lg:grid-cols-4">
          {statCards.map(card => (
            <div
              key={card.label}
              className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/70 p-5 shadow-sm space-y-3"
            >
